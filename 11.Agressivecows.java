...
public class 11.Agressivecows {

    public int solve(int[] arr,int cows){
        Arrays.sort(arr);
      int upperbound= Arrays.stream(arr).max().getAsInt();        //linear search
        for(int i =1;i<upperbound;i++){
        if(ifpossible(arr,cows,i))
            max=Math.max(max,i);
        }
        //binary search
        int low=1;
        while(low<=upperbound){
            int mid=(low+upperbound)/2;

            if(ifpossible(arr, cows, mid)){
                max=mid;
                low=mid+1; // got the max and want to check there exists a max;
            }
            else
               high = mid-1;
        }
    ...
    public boolean ifpossible(int[] arr, int cows, int mindistance){
       int cntCows=1;
        int cowposition=arr[0];
        for(int i =0;i<arr.length;i++){
            if(arr[i]-cowposition>=mindistance){
                cntcows++;
                //update the cow
                cowposition=arr[i];
            }
